...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while k <= num_rolls:
        ...
        k += 1
    return 1 if pig_out else result
    # END PROBLEM 1
...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    assert player_score >= 0 and opponent_score >= 0, 'Score must be non-negative'

    ones = player_score % 10
    tens = (opponent_score // 10) % 10
    result = 3 * abs(ones - tens)
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score in range(goal):
        for opp_score in range(goal):
            if strategy(score, opp_score) != first:
                return False
    return True
    # END PROBLEM 7
